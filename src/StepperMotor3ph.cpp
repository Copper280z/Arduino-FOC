...
StepperMotor3ph::StepperMotor3ph(int pp, float _R, float _KV, float _inductance)
: FOCMotor()
{
...
*/
void StepperMotor3ph::linkDriver(BLDCDriver* _driver) {
  driver = _driver;
}

// init hardware pins
void StepperMotor3ph::init() {
...
int  StepperMotor3ph::initFOC( float zero_electric_offset, Direction _sensor_direction ) {
  int exit_flag = 1;
...
int StepperMotor3ph::alignSensor() {
...
// - to the index
int StepperMotor3ph::absoluteZeroSearch() {
...
void StepperMotor3ph::move(float new_target) {
...
void StepperMotor3ph::setPhaseVoltage(float Uq, float Ud, float angle_el) {
  ...
  Umin = fmin(fmin(Ualpha, Ubeta), 0);
  Umax = fmax(fmax(Ualpha, Ubeta), 0);
  Vo = -(Umin + Umax)/2 + voltage_limit/2;

  Ualpha = Ualpha + Vo;
  Ubeta = Ubeta + Vo;

  // set the voltages in hardware
  driver->setPwm(Ualpha, Ubeta, Vo);
}

// Function (iterative) generating open loop movement for target velocity
// - target_velocity - rad/s
// it uses voltage_limit variable
float StepperMotor3ph::velocityOpenloop(float target_velocity){
...
float StepperMotor3ph::angleOpenloop(float target_angle){
  // get current timestamp
  unsigned long now_us = _micros();
  // calculate the sample time from last call
  float Ts = (now_us - open_loop_timestamp) * 1e-6f;
